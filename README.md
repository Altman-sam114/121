...
- **v2.4 武将道路/交战规则**：`GeneralAssignment` 保存武将姓名、风格和技能快照；`GeneralInfluence` 让武将影响 `MovementRules` 的道路机动上限，以及 `CombatRules` 的攻击/防御修正；`CommandExecutor` 会在移动日志中追加中文武将道路机动摘要，在攻击和反击日志中追加中文武将姓名与攻防修正摘要，并通过 `CombatAuditSummary` 输出攻击/防御有效值、地形、河流、器械攻城、围城、死守和侧击等交战因素；`GeneralCommandPanelView` 也会显示当前选中防区的道路机动与接敌攻防只读摘要；核心移动、攻击、反击、姿态、回合推进、动态方面事件日志和命令结果/拒绝原因已开始中文化；道路敌控区、攻击目标、粮道阻断和安全补员邻接统一按 `Faction.isHostile(to:)` 判断，不再靠旧二元 `!= faction` 推断敌我。
