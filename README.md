...
- **v2.4 武将道路/交战规则**：`GeneralAssignment` 保存武将风格和技能快照；`GeneralInfluence` 让武将影响 `MovementRules` 的道路机动上限，以及 `CombatRules` 的攻击/防御修正；`CommandExecutor` 会在战斗和反击日志中追加武将攻防修正摘要；道路敌控区、攻击目标、粮道阻断和安全补员邻接统一按 `Faction.isHostile(to:)` 判断，不再靠旧二元 `!= faction` 推断敌我。
