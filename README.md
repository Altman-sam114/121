...
- **v2.4 武将复核与战术塑形层**：`GeneralAgent.plan` 读取 `FrontZone.generalAssignment` 与武将 registry，对军师后的 `ZoneDirective` 做忠诚/满意度/风格收束，并按武将技能把攻势塑形为突击、破阵、合围、箭雨/器械压制或佯攻，把守势塑形为固守、诱敌/退守、层层设防或死守；它只返回调整后的 `DirectiveEnvelope` 和 `GeneralDecisionRecord`，不绕过执行器，也不直接移动军队。
