...
- `CommandExecutor` 会把中文武将姓名、道路机动、交战审计和攻防修正摘要写入移动、攻击和反击日志；`GeneralCommandPanelView` 会只读显示当前防区道路机动与接敌攻防摘要；`UnitInspectorView` 会通过 `AppContainer.selectedUnitMobilityPreviewNotes` 对选中军队显示基础/有效机动、武将官道加成、可达格数和官道状态，通过 `selectedUnitCombatPreviewNotes` 显示最多三名射程内敌军的预计伤害、反击风险和距离排序，首选目标继续显示武将影响和交战审计；`RegionInspectorView` 会显示当前郡县官道覆盖和当前地格官道状态；这些 UI 只读复用移动/交战/地图派生计算，不执行命令；核心移动、攻击、反击、姿态、回合推进、动态方面事件日志和命令结果/拒绝原因已开始中文化，便于审计“武将做了什么、命令为什么失败”。
...
flowchart TD
    ...
    UMP["军队道路机动预判<br/>AppContainer.selectedUnitMobilityPreviewNotes<br/>基础/有效机动 + 官道状态 + 可达格"]:::ui
    UCP["军队接战预判<br/>AppContainer.selectedUnitCombatPreviewNotes<br/>最多三目标排序 + 首选武将影响 + 交战审计"]:::ui
