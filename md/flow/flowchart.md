...
- `RuleEngine` 成功命令结果、`AppContainer` 玩家交互日志和 `AgentPanelView` 命令结果标题已使用三国语义展示名：移动/交战使用军队展示名，修路显示郡县，外交显示国家或势力，命令结果标题显示命令类别；旧 `RegionCommand.displayName`、`CommandIntentAdapterError` 和 `TheaterDirectiveDecoderError` 的可见说明也已收口为中文郡县/军师指令诊断，不把 raw region / division / front zone id 当作玩家文案；底层 `Command.displayName`、记录字段、军师指令 JSON 和 Codable schema 保持兼容。
