...
- `CommandExecutor` 会把中文武将姓名、道路机动、交战审计、攻防修正摘要和结算后剩余兵力写入移动、攻击和反击日志；`BoardScene` 会只读 `PlayerCommandState.plannedOperations`，把玩家宏观军令画成源点、目标点、进攻箭头或固守圆环，并显示武将/攻守/短战术、源目锚点官道状态和敌军压迫短标签；`GeneralCommandPanelView` 会只读显示当前防区道路机动、官道受益军队、接敌攻防、最近敌军对象和麾下军队兵力/粮草/军令/行动状态摘要，也会通过 `AppContainer.selectedGeneralPlannedOperationRows` 在计划军令列表显示武将、最终战术、源→目标、官道状态、源目受压和最近敌距；`UnitInspectorView` 会通过 `UnitInspectorStrategicState.generalAssignment` 显示选中军队所属武将、风格、忠诚/满意和中文技能摘要，通过 `AppContainer.selectedUnitMobilityPreviewNotes` 对选中军队显示基础/有效机动、武将官道加成、可达格数、当前位置/郡县官道状态、本回合可达官道、安全官道和受敌控区压迫的官道格数，通过 `selectedUnitCombatPreviewNotes` 显示最多三名射程内敌军的首选理由、接战官道压制位、首选交战武将、候选敌将、候选武将修正、预计伤害、结算后敌我剩余兵力、撤退/死守/断粮额外损失/歼灭风险、反击风险、目标地形/城关/官道/粮道/撤退姿态和距离排序；无射程内敌军但存在敌对军队时显示最近敌军、接近候选、候选可达距/安全官道/入敌射风险、接近武将、非零接近参考修正、接近态势、接近威胁、需接近格数、可达更近官道位和抵达官道后是否入射程；无 hostile 敌军时显示当前无可预判敌对军队空状态；首选目标继续显示武将影响和交战审计；`RegionInspectorView` 会显示当前郡县官道覆盖、可见敌军造成的官道受压、本郡武将、可见敌军距离/射程/兵力/敌将、当前地格官道状态、可见敌对军队和可见非敌对军队；这些 UI 只读复用移动/交战/地图派生计算，不执行命令；核心移动、攻击、反击、姿态、回合推进、动态方面事件日志和命令结果/拒绝原因已开始中文化，便于审计“武将做了什么、命令为什么失败”。
...
flowchart TD
    ...
    GIP["武将影响面板摘要<br/>GeneralCommandPanelView<br/>道路机动 + 官道受益军队 + 接敌攻防 + 近敌对象 + 麾下备战"]:::ui
    UGA["军队所属武将摘要<br/>UnitInspectorStrategicState.generalAssignment<br/>姓名 + 风格 + 忠诚满意 + 技能"]:::ui
    UMP["军队道路机动预判<br/>AppContainer.selectedUnitMobilityPreviewNotes<br/>基础/有效机动 + 可达格 + 可达/安全官道"]:::ui
    UCP["军队接战预判<br/>AppContainer.selectedUnitCombatPreviewNotes<br/>无敌对空状态 + 最近敌军 + 接近候选可达/官道/入敌射 + 接近武将 + 接近参考 + 接近态势 + 接近威胁 + 官道接近 + 官道入射程判断 + 首选理由 + 接战官道 + 交战武将 + 候选敌将 + 候选武将修正 + 三目标排序 + 敌我余兵 + 目标态势 + 撤退/歼灭风险 + 首选武将影响 + 交战审计"]:::ui
    RRP["郡县态势摘要<br/>RegionInspectorState / RegionInspectorView<br/>官道覆盖/受压 + 本郡武将 + 敌军距离/射程/兵力/敌将"]:::ui
    POM["计划军令反馈<br/>BoardScene + GeneralCommandPanelView<br/>源/目标点 + 进攻箭头/固守环 + 武将/战术/官道受压/最近敌距摘要"]:::ui
...
flowchart TD
    ...
    BOARD["视觉反馈<br/>BoardScene + GeneralCommandPanelView<br/>进攻箭头、防御圆环、武将战术/官道受压/最近敌距摘要、微操单位金色圈"]:::ui
