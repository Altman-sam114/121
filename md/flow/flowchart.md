...
- 道路敌控区、玩家地图点击攻击、可见攻击高亮、武将宏观目标选择、AI/MockAI 单位敌军摘要、`WarCommandExecutor` 单位敌军强度/存在/目标筛选、粮道阻断和安全补员邻接统一使用 `Faction.isHostile(to:)` 判断敌对；郡县检查器的可见敌军/非敌对军队分组、官道受压计数和最近来源优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺建档时 fallback 到 `Faction.isHostile(to:)`；只读预判和高亮还会按玩家视角可见性过滤；中立不会只因不是当前阵营就阻断道路、影响 AI 威胁或成为合法攻击目标。
