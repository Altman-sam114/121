...
- 规则、AI 摘要、MockAI 威胁估算和 `WarCommandExecutor` 单位目标筛选中的敌对判断优先用 `Faction.isHostile(to:)`，不在新代码里继续依赖二元 `opponent`。
...
- 道路敌控区、玩家地图点击攻击、可见攻击高亮、武将宏观目标选择、AI/MockAI 单位敌军摘要、`WarCommandExecutor` 单位敌军强度/存在/目标筛选、`WarDeploymentManager` 单位级敌军存在和相邻敌对防区接触、`RegionCommandValidator` 攻击目标校验、粮道阻断和安全补员邻接统一使用 `Faction.isHostile(to:)` 判断敌对；郡县检查器的可见敌军/非敌对军队分组、官道受压计数和最近来源，`AppContainer` 的军队接战预判、武将麾下/目标/计划军令近敌与受压摘要、移动预览可见敌控过滤和点击日志敌对措辞，以及 `BoardScene` 计划军令地图短标签、`CommandPanelView` 状态文案、`GeneralCommandPanelView` 目标预览显示 gate，优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺建档时 fallback 到 `Faction.isHostile(to:)`；只读预判和高亮还会按玩家视角可见性过滤；中立不会只因不是当前阵营就阻断道路、影响 AI 威胁或成为合法攻击目标。
