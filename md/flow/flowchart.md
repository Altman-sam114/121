...
- `AgentPanelView` 的主审计字段、君主/外交官/太守/军师/武将记录和防区指令摘要优先使用展示名；外交对象来自 `CountryProfile.name`，郡县来自 `RegionNode.name`，防区来自 `FrontZone.name` 或 `曹军防区：官渡、许昌` 这类只读 fallback，调试 JSON 和 Codable raw id 保持原样。
- `AppContainer.interactionLog` 的玩家可见文本也已中文优先：基础命令执行/拒绝、武将军令提交/拒绝、规则拒绝摘要、命令条数、手动指挥军队排除、查看/选择军队和选择地格/郡县都显示为三国语义；底层命令、记录 id、Codable 和 rawValue 不变。
...
flowchart TD
    ...
    AIUI["AI 面板<br/>AgentPanelView<br/>Agent/国家/郡县/防区展示名 + 调试 JSON + command results + zone directives"]:::ui
