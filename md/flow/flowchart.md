# 三国棋策 Agent Mermaid 核心流程图（v2.5 UI 收口起步，v2.4 君主/外交/太守/军师/武将规则主线）

> 本图参照 `md/flow/flow.md`。项目正从 `WWIIHexV0` 二战原型迁移到三国题材；v2.5 已开始发布级三国 UI、美术和交互收口，当前完成共享 `SanguoDesignTokens` 起步、HUD/战报/主覆盖层/地图底色接入，以及 MapEditor 默认三国军队模板和编辑器导出文案小步；v2.4 规则主线当前完成官渡默认剧本预览、三国兵种模板兼容层、战术审计显示三国化、围城/粮草最小规则、兵种克制最小规则和君主/外交/太守/军师/武将指令编排、外交与太守生产命令、武将战术塑形、道路、粮道与交战兼容层，图中仍保留 `Division`、`Faction`、`Theater`、`FrontZone` 等代码名，中文解释已按三国迁移口径理解为军队、势力、方面、防区。
...
- `ScenarioDefinition.victoryConditions` 中 active 的 `controlObjective` 会随加载进入 `VictoryState.scenarioConditions`；`VictoryRules` 优先按 objective id 对应 hex controller 判断官渡剧本目标，HUD 会显示中文胜利原因；`RegionVictoryRules` / `RegionRuleSystem` 只读分析也复用同一剧本条件，fallback 时同样读取旧 objective id，旧阿登胜负枚举和 fallback 规则保留兼容。
...
- 规则、AI 摘要、MockAI 威胁估算和 `WarCommandExecutor` 单位目标筛选中的敌对判断不在新代码里继续依赖二元 `opponent`；攻击合法性、玩家点击/高亮、道路 ZOC、粮道单位阻断、粮道控制格通行、撤退安全格控制格、宏观军令落点敌控优先级、宏观动态方面推进、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、动态前线敌对接触、ZoneCommanderAgent / MarshalBattlefieldSummarizer / RulerStrategicSnapshot / StrategistBattlefieldSnapshot 单位级 AI hostile 摘要、执行器单位目标筛选和区域交战压力优先使用 `DiplomacyState` 的 hostile / atWar 口径，缺外交建档时 fallback 到 `Faction.isHostile(to:)`。
...
- `EconomyRules` 的事件日志已把经济启动、排产、府库结算、战略粮草短缺、自动补员、粮草辎重完成、生产军队部署、修路结果和无安全后方部署格改为中文三国语义；资源摘要显示人口、军械、粮草，修路和部署位置优先显示城池、关隘、郡县、官道或地形，坐标只作次级定位，规则数值、生产队列、修路选点和部署筛选不变。
...
- Legacy Agent D 的 parser / mapper 可见错误说明已改为中文军机、军令、军队和目标郡县语义；缺目标错误不再插入 raw 军队 id；JSON 机器字段、schema 和命令管线不变，默认战争 AI 主路径不退回 Legacy Agent D。
...
flowchart TD
    ...
    WCE["指令翻译器<br/>WarCommandExecutor<br/>把战区意图翻成具体单位命令<br/>生成道路/交战审计诊断<br/>道路审计含源目官道/接敌/受压/路径成本/超限/无通路/占位边界<br/>交战审计含目标兵力/地形/官道/粮草/姿态"]:::command
