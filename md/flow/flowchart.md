...
- `CommandExecutor` 会把中文武将姓名、道路机动、交战审计和攻防修正摘要写入移动、攻击和反击日志；`GeneralCommandPanelView` 会只读显示当前防区道路机动与接敌攻防摘要；`UnitInspectorView` 会通过 `AppContainer.selectedUnitCombatPreviewNotes` 对选中军队显示预计伤害、反击、武将影响和交战审计，只读复用交战计算，不执行命令；核心移动、攻击、反击、姿态、回合推进、动态方面事件日志和命令结果/拒绝原因已开始中文化，便于审计“武将做了什么、命令为什么失败”。
