...
- `CommandExecutor` 会把武将攻防修正摘要写入攻击/反击日志，便于审计“武将做了什么”。
...
flowchart TD
    ...
    LOG["日志和复盘记录<br/>EventLog / WarDirectiveRecord / AgentDecisionRecord / RulerDecisionRecord<br/>含武将攻防修正摘要"]:::ui
    ...
    PLAYER --> CMD
    AI --> DEC --> COMP --> RULER --> DIPLO --> DCMD --> GOV --> GCMD --> STRAT --> GENA --> ZD --> WCE --> CMD
    DCMD --> CMD
    GCMD --> CMD
    GENA --> GINF
    GINF --> WCE
...
flowchart TD
    ...
    ENV --> RULER --> DIPLO --> DCMD --> DRE --> GOV --> GCMD --> GRE --> STRAT --> GENA --> TACTIC --> WCE --> BOTTOM --> RE --> RECORD --> END
    DRE --> DREC
    GRE --> GOVREC
    GENA --> GINF
    GINF --> WCE
