...
- 道路敌控区、玩家地图点击攻击、攻击高亮、武将宏观目标选择、粮道阻断、安全补员邻接和郡县可见敌军展示都使用 `Faction.isHostile(to:)` 判定敌对，避免中立或后续多势力数据被旧二元 `!= faction` 误判为敌军。
...
- `CommandValidationError` 保留英文 rawValue 作为 Codable / 测试兼容身份，同时提供中文 `displayName`；`RuleEngine`、`WarCommandExecutor`、`TurnManager` 和 `AgentDecisionRecord` 使用中文展示值写入 `CommandResult`、事件日志、`WarDirectiveRecord.diagnostics` 和 AI 面板命令结果；`CommandPanelView.statusText` 按 `Faction.isHostile(to:)` 区分敌军和非敌对军队，但下令权限仍只给玩家当前回合的己方未行动军队。
...
- `MovementRules.isEnemyZoneOfControl`、`CommandValidator.validateAttack`、`RegionCommandValidator` 攻击目标校验、`WarCommandExecutor` 单位级敌军强度/存在/目标筛选、`WarDeploymentManager` 单位级敌军存在和相邻敌对防区接触、`RulerAgent` / `MockAICommander` / `ZoneCommanderAgent` 单位级敌军摘要、`AppContainer` 的地图点击攻击/可见攻击高亮/武将宏观目标选择、`SupplyRules.canSupplyPass` 和 `EconomyRules` 的安全补员邻接判断当前都已经接入 `Faction.isHostile(to:)`；只读预判和高亮还会通过 `MapDisplayAdapter` 过滤玩家视角可见敌军；郡县检查器的可见敌军/非敌对军队分组、官道受压计数和最近来源，`AppContainer` 的军队接战预判、武将麾下/目标/计划军令近敌与受压摘要、移动预览可见敌控过滤和点击日志敌对措辞，以及 `BoardScene` 计划军令地图短标签、`CommandPanelView` 状态文案、`GeneralCommandPanelView` 目标预览显示 gate，优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺建档时 fallback 到 `Faction.isHostile(to:)`；非己方控制 hex/region 的部署压力分类仍表示未控制或争夺，不等同于敌军单位；完整借道/同盟通行仍待后续外交制度。
