...
MapEditor / JSON 数据
  ...
  -> Region 聚合
  -> VictoryState.scenarioConditions / VictoryRules 剧本目标判断
...
- `GeneralAssignment` 现在保存武将姓名、风格和技能快照；`GeneralSkillDisplay` 将 raw skill id 显示为粮道调度、骑兵突击、守备专精等中文标签，并为 UI 提供官道机动、骑兵突击、攻城修正、地形/渡河防御等只读短效果提示，规则和 JSON 仍保留原 id；`GeneralAgent` 会读取这些快照，把军师后的 `ZoneDirective.tactic` 收束为攻守类别合法、且符合机动/器械/预备队条件的战术；`GeneralInfluence` 会读取防区武将分配，给道路机动、攻击和防御提供小幅规则修正，并通过 `GeneralMovementInfluenceSummary.noBonusReason` 同源解释未分配武将、忠诚/满意不足、缺道路技能或所在郡县无可借官道等道路无加成原因。
- `WarCommandExecutor` 会在宏观军令选兵排序中使用 `MovementRules.effectiveMovementLimit`，并在 `tacticalDestination` 中用 `MovementRules.shortestPath` 判断目标郡县候选 hex 是否可达；目标郡县候选和接近候选的“敌控格优先”排序也按 `DiplomacyState` hostile / atWar 判断，让武将官道机动、道路、外交敌对控制格、敌控区、地形和占位共同影响 AI / 玩家武将宏观军令落点；执行器还会把每条防区指令最多前三条宏观命令的道路审计和交战审计写入 `WarCommandExecutionResult.diagnostics`，由 `TurnManager` 和玩家武将军令记录合并进 `WarDirectiveRecord.diagnostics`，`AgentPanelView` 先按原始“道路审计 / 交战审计”marker 分组，再把它们拆到防区指令卡片内的“战术审计”行并按官道/交战图标展示，若安全过滤命中则使用“道路审计已记录 / 交战审计已记录”类型化兜底；这些审计在不改变命令行为的前提下显示军队基础/有效机动、官道加成或无加成原因、源/移动目标/攻击目标据道或离道、hostile 接敌或邻接受压、目标郡县、路径成本/超限/无通路/占位边界、攻击目标、预计伤害、反击风险、目标兵力/地形/官道/粮草/姿态和 `CombatRules.combatAuditSummary` 复用的攻防因素；`CommandExecutor` 会把 `GeneralInfluence` 的道路机动摘要追加到移动日志，把交战审计、攻防修正摘要和结算后剩余兵力追加到攻击和反击日志，日志片段中文优先并优先显示武将姓名，缺少姓名快照时显示“未命名武将”而不展示 raw `generalId`；`BoardScene` 会只读 `PlayerCommandState.plannedOperations`，把玩家宏观军令画成源点、目标点、进攻箭头或固守圆环，并显示武将/攻守/短战术、源目锚点官道状态、可见敌军压迫和最近可见敌军短标签；`AppContainer.selectedGeneralPlannedOperationRows` 会基于同一计划记录和源/目标代表 hex 生成武将、最终战术、源→目标、源/目标据道或离道、源目可见受压和最近可见敌军对象/距离摘要，供 `GeneralCommandPanelView` 的计划军令列表展示，缺少武将、郡县或防区展示资料时使用中文占位，不把 raw id 当作玩家文案 fallback；尚未提交进攻郡县时，`AppContainer.selectedGeneralTargetPreviewNotes` 会基于当前选中目标郡县生成源/目标代表 hex 的官道据守/离道、可见 hostile 邻近受压和最近可见敌军距离摘要，`GeneralCommandPanelView` 只负责在目标名下展示；`GeneralCommandPanelView` 还会只读展示当前选中武将防区的道路机动、官道受益军队或无加成原因、可见接敌攻防、当前可见接敌配对、最近可见敌军对象和麾下军队兵力/粮草/军令/行动/官道/接敌状态摘要，并在武将技能行显示短效果提示；当“固守战线”或“进攻郡县”按钮不可用时，`AppContainer` 生成观察模式、玩家阶段、防区或目标条件不足的只读原因，`GeneralCommandPanelView` 只负责展示原因而不重新推导规则；`UnitInspectorView` 会读取 `UnitInspectorStrategicState.generalAssignment`、`AppContainer.selectedUnitMobilityPreviewNotes` 和 `selectedUnitCombatPreviewNotes`，对当前选中军队显示所属武将、风格、忠诚/满意、带短效果提示的中文技能摘要、基础/有效机动、武将官道加成、玩家视角可见可达格数、当前位置官道可见受压或郡县官道状态、本回合可达官道、最近可达/可见安全官道距离、可见安全官道和受可见敌控区压迫的官道格数，以及最多三名玩家视角可见射程内敌军的首选理由、可见接战官道压制位、首选交战武将、候选敌将、候选武将修正、候选交战审计、预计伤害、结算后敌我剩余兵力、撤退/死守/断粮额外损失/歼灭风险、反击风险、目标地形/城关/官道/粮道/撤退姿态和距离排序；无可见射程内敌军但存在可见敌对军队时同一小节会显示最近可见敌军、接近候选、候选可达距/可见安全官道/入敌射风险、接近武将、非零接近参考修正、接近态势、接近威胁、需接近格数、可达更近官道位以及抵达官道后是否入射程；无可见 hostile 敌军时显示当前无可预判敌对军队空状态；首选目标继续显示武将影响详情，前三个候选目标行各自显示交战审计；`RegionInspectorView` 会从 `RegionInspectorState` 展示当前郡县官道覆盖、可见敌军造成的官道受压和最近来源、本郡武将、可见敌军相对选中地格或郡县代表格的距离/射程/兵力/敌将、当前地格官道状态、可见敌对军队、可见非敌对军队及其势力/外交关系/紧张度摘要，方便玩家判断修路、行军、粮道、交战和多势力态势；这些预览复用 `MovementRules` / `CombatRules` / `GeneralInfluence` 或 `MapDisplayAdapter` 的只读派生，不执行命令、不修改状态；核心移动、攻击、反击、姿态、回合推进、动态方面事件日志和命令结果/拒绝原因已开始中文化，便于玩家和 Agent C 复判“哪个武将影响了道路与交战、哪个命令为什么被拒绝”。
- `RootGameView`、`NewGameButton`、macOS `CommandMenu`、`InfoPanelToggle`、`AgentPanelView` 和 `AppContainer.interactionLog` 当前还负责玩家入口与交互日志的三国化显示：新局、结束回合、军情、军机谋议、执行者、军令 fallback、军机回合、本地 mock / no-op 来源、兼容 fallback 指挥者名称、基础命令执行/拒绝、武将宏观军令提交/拒绝、规则拒绝、命令条数、手动指挥军队排除、查看/选择军队和选择地格/郡县均中文优先；军队点击日志优先使用 `Division.thematicDisplayName`，这不改变底层命令、记录 id、Codable、调试 JSON 或 rawValue。
- 军队详情、玩家选择地格日志、移动命令结果、行军日志、动态方面推进事件和地图计划军令标签当前使用城池、关隘、郡县、官道或地形优先的中文地格展示，坐标只作括号内次级定位；缺武将展示名时显示“未命名武将”，不把 `generalId` 直接放到地图标签；战报 metadata 有 `relatedRecordId` 时只显示“军机审计”，不直接显示 raw record id；外交面板君主/外交官记录字段显示“执行者”而不是英文 `Agent`。这只改变可见文本，不改变 `HexCoord`、`GameLogEntry.relatedRecordId`、`PlayerPlannedOperation.createdByGeneralId`、Codable/rawValue、移动、道路、交战或外交规则。
...
- `SupplyRules` 的玩家可见事件日志当前已覆盖整补、围城恢复阻断、撤退、撤退失败、粮道断绝围城损耗、包围损耗和撤退整顿中文化，并使用 `Division.thematicDisplayName`；撤退位置优先显示城池、关隘、郡县、官道或地形，坐标只作次级定位；这只改变 `GameState.appendEvent(...)` 文案，不改变补给路径、围城判定、撤退目的地、损耗数值或事件类别。
- `EconomyRules` 的玩家可见事件日志当前已覆盖经济账本启动、排产成功/失败、回合府库结算、战略粮草耗尽、自动补员、粮草辎重入库、生产军队后方部署和无安全后方部署格中文化；资源摘要统一为人口、军械、粮草，生产部署优先显示军队展示名、郡县后方和地名优先的安全格摘要，修路结果优先显示城池、关隘、郡县、官道或地形，坐标只作次级定位；这只改变 `GameState.appendEvent(...)` 文案，不改变收入、维护费、补员、修路选点、生产部署、安全后方筛选或事件类别。
...
- `RuleEngine`、`AppContainer`、`CommandResultSummary` 和 `AgentPanelView` 的玩家/AI 可见命令结果展示当前已收敛：成功 `CommandResult.message` 使用 state-aware 中文摘要，玩家交互日志不再重复拼接 raw id 版 `Command.displayName`，新生成的 AI 命令结果记录使用 `Command.auditDisplayName` 保存进军、交战、固守、修路、外交等中文安全类别，AI 面板命令结果标题只显示命令类别；`Command.displayName` 本体仍保留 raw id / 坐标兼容输出，旧 `CommandResultSummary.commandDisplayName` 记录和旧 AI 兼容判断通过 `commandDisplayNameForDisplay` 继续映射为中文类别。
- `RegionCommand.displayName`、`CommandIntentAdapterError`、`DirectiveTarget` 解码错误和 `TheaterDirectiveDecoderError` 的玩家/AI 可见诊断当前已收口为中文三国语义：旧 region 命令只显示郡县进军/交战/固守/补给类别，region-to-hex adapter 错误不拼军队或郡县 raw id，军师指令 JSON 解码/校验失败说明使用防区、方面、郡县、战术等中文术语；`TurnManager` 外交和太守命令成功诊断复用 `CommandResult.message`。底层 `RegionCommand`、`TheaterDirective` JSON、Codable schema、rawValue、命令校验和执行规则不变。
- 道路敌控区、粮道上的敌对单位阻断、粮道控制格通行、撤退安全格控制格、战略郡县粮道控制区通行、撤退安全郡县、宏观军令落点敌控优先级、宏观动态方面推进、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、动态前线敌对接触、Legacy AgentContext 敌军摘要、MockAICommander 威胁估算、ZoneCommanderAgent / MarshalBattlefieldSummarizer / RulerStrategicSnapshot / StrategistBattlefieldSnapshot 单位级 AI hostile 摘要、玩家地图点击攻击、攻击高亮、武将宏观目标选择、攻击合法性、执行器单位目标筛选、区域交战压力和郡县可见敌军展示优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺外交建档时 fallback 到 `Faction.isHostile(to:)`，避免中立、停战、同盟或共同作战势力被误判为可攻击、切粮、阻断战略郡县粮道、阻止战略撤退安全郡县、围城、阻止撤退安全格、阻止补员、生成敌对部署接触、生成敌对前线、推进宏观动态方面或进入 AI 敌军摘要的敌军；objective / region controller 来源、非同 faction 堆叠阻挡、非己方控制 hex / region 的部署压力分类、前线压力/补给/包围拓扑和 encirclement 拓扑仍保留原控制权/阵营语义；移动后自动占领的 hex controller 改写单独由 `OccupationRules.canOccupy` 按外交 hostile / atWar gate 收束；`CommandExecutor` 直接命令路径的动态推进谓词仍待后续 parity 切片同步。
...
- Legacy Agent D 的 `AgentDecisionParserError` 和 `AgentCommandMappingError` 可见错误说明当前已改为中文军机/军令语义；缺少目标地格、目标郡县或目标军队时不再把 raw `divisionId` 插入错误文案；`schemaVersion`、`agentId`、`turn`、`toRegionId`、`targetDivisionId` 等机器字段、parser / mapper 控制流和命令管线保持兼容，Legacy Agent D 仍只作回归参考。
...
- `ZoneCommanderAgentConfig.CommandStyle.displayName` 是武将统军风格共享展示入口；`GeneralAssignment.commandStyleDisplayName` 会在快照 raw 值缺失或损坏时按忠诚/满意推断进取、持重或谨慎，和 `GeneralAgent` 实际执行口径一致。武将军令面板、武将档案、军队详情和军机面板不再各自维护风格显示 switch；`TheaterCommanderPool` 和模拟元帅进入军机面板的防区/方面指令摘要与 fallback diagnostics 也已中文化；`TurnManager` 的武将层复核事件、君主/太守/军师诊断、计划军令摘要和 `AgentPanelView` 武将记录缺少展示名时使用中文占位，不把 `generalId`、`RegionId` 或 `FrontZoneId` rawValue 作为玩家文案 fallback；这些都不改变 directive schema、记录 id 或执行规则。
...
- 规则/AI 摘要、MockAI 威胁估算和 `WarCommandExecutor` 单位目标筛选中的敌对判断不应继续依赖二元 `Faction.opponent`；`Faction.opponent` 只作为旧兼容 helper，不应作为新代码敌我关系来源。
- `MovementRules.isEnemyZoneOfControl`、`SupplyRules` 粮道单位阻断、粮道控制格通行、撤退安全格控制格和围城邻接、`RegionSupplyRules` 战略郡县粮道控制区通行和撤退安全郡县、`EconomyRules` 安全补员邻接、`WarDeploymentManager` 单位级敌军存在和相邻敌对防区接触、`FrontLineManager` 动态相邻 theater 是否形成敌对前线、`AgentContextBuilder` 的 `enemyDivisions` / enemy supply 摘要、`MockAICommander.visibleEnemyStrength`、`ZoneCommanderAgent` / `MarshalBattlefieldSummarizer` / `RulerStrategicSnapshot` / `StrategistBattlefieldSnapshot` 单位级 AI hostile 摘要、`CommandValidator.validateAttack`、`RegionCommandValidator` 攻击目标校验、`CommandExecutor.executeAttack` 防绕过 guard、`WarCommandExecutor` 单位级敌军强度/存在/目标筛选、宏观军令落点敌控优先级和宏观动态方面推进、`RegionCombatRules.pressure`、`AppContainer` 的地图点击攻击/可见攻击高亮/武将宏观目标选择、郡县检查器的可见敌军/非敌对军队分组、官道受压计数和最近来源，`AppContainer` 的军队接战预判、武将麾下/目标/计划军令近敌与受压摘要、移动预览可见敌控过滤和点击日志敌对措辞，以及 `BoardScene` 计划军令地图短标签、部署图层角色 fallback、`CommandPanelView` 状态文案、`GeneralCommandPanelView` 目标预览显示 gate，优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺建档时 fallback 到 `Faction.isHostile(to:)`；objective 和 region controller 来源仍保持 controller / occupation 事实，只在 hostile 分类时使用外交关系；`MovementRules` 的非同 faction 堆叠阻挡、`WarDeploymentManager` 非己方控制 hex / region 的部署压力分类、`FrontLineManager` 的 pressure / supplyImpact / encirclementCandidate 和 encirclement 出口/接触拓扑继续使用控制权或阵营语义，不等同于敌军单位；完整借道/同盟通行、共同作战堆叠和共享补给源属于后续外交制度。
...
对每个 active theater:
  对 theater.regionIds 中的每个 region:
    ...
    如果邻接 hex 属于另一个 dynamic theater
       且对方 theater 的 sourceFaction 与 friendlyFaction 在 DiplomacyState 中 hostile:
         形成 enemy region 接触
...
- 在 `GeneralCommandPanelView`、`GeneralProfileView` 和 `UnitInspectorView` 的 UI 展示中提供只读短效果提示，例如官道机动、骑兵突击、攻城修正、地形/渡河防御；这些提示不参与规则判断。
...
- `AppContainer.selectedGeneralInfluenceNotes` 复用 `GeneralInfluence.movementSummary`、`GeneralInfluence.combatSummary` 和当前军队坐标，为 `GeneralCommandPanelView` 提供当前防区道路机动、官道受益军队或无加成原因、可见接敌攻防、当前可见接敌配对与最近可见敌军对象只读摘要；它不执行命令，也不修改道路、军队、控制权或动态战区。
- `MapDisplayAdapter.unitInspectorState` 会用同一套优先级为选中军队附带 `GeneralAssignment` 快照：先找显式分配到该军队的武将，再 fallback 到当前 hex 所属防区武将；`UnitInspectorView` 只读展示武将姓名、风格、忠诚/满意、带短效果提示的中文技能和玩家干预次数，不改变武将分配或规则计算。
- 道路 ZOC、粮道上的敌对单位、粮道控制格通行、撤退安全格控制格、战略郡县粮道控制区通行、撤退安全郡县、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、动态前线敌对接触、Legacy AgentContext 敌军摘要、MockAICommander 威胁估算、ZoneCommanderAgent / MarshalBattlefieldSummarizer / RulerStrategicSnapshot / StrategistBattlefieldSnapshot 单位级 AI hostile 摘要、攻击目标、玩家高亮/点击、执行器单位目标、宏观军令落点敌控优先级和区域交战压力优先按 `DiplomacyState` 的 hostile / atWar 判断，缺外交建档时 fallback 到 `Faction.isHostile(to:)`；`OccupationRules.canOccupy` 也只允许占领无控制者或外交 hostile / atWar 控制格，AI / 武将执行器只把执行前真实可占领的移动作为动态方面突破依据，避免非敌对借道经过 allied / coBelligerent / neutral 控制格时翻转 controller 或推进动态归属；objective 和 region controller 来源、非同 faction 堆叠阻挡、前线压力/补给/包围拓扑和 encirclement 拓扑仍保留原语义。中立势力不会只因 `faction != activeFaction` 阻断道路、切断粮道、阻断战略郡县粮道、阻止战略撤退安全郡县、触发围城、阻止补员、生成敌对部署段、生成敌对前线、进入 AI 敌军摘要、被自动占领或被合法攻击。
...
`RootGameView` / HUD / macOS 菜单 / `AgentPanelView` / `AppContainer.interactionLog` 的玩家可见文本已覆盖新开战局、结束回合、军情开关、军机谋议、执行者、军令 fallback、军机回合、本地 mock / no-op 来源、兼容 fallback 指挥者名称、基础命令执行/拒绝、武将军令提交/拒绝、规则拒绝摘要、命令条数、手动指挥军队排除、查看/选择军队和选择地格/郡县的中文三国化显示；这些只是入口文案、`appendInteractionEvent(...)` 与私有展示 helper 的文本边界，`submitPlayerDirective(...) -> GeneralAgent.plan -> WarCommandExecutor -> RuleEngine` 的执行顺序、`WarDirectiveRecord`、`PlayerPlannedOperation`、Codable 字段、调试 JSON 和 rawValue 不变。
...
这条路径中的交互日志只做展示：成功/拒绝状态写作“命令已执行/命令被拒绝”，玩家可见命令摘要来自 state-aware `CommandResult.message` 或安全命令类别；`Command.displayName` 仍保留兼容 raw 文案，不应直接作为新 UI 文案来源。
...
武将面板宏观军令的交互日志写作“武将军令失败/已提交/已执行”，并用中文记录未生成可执行命令、命令被规则拒绝、已手动指挥军队被排除等诊断；这些诊断只进入玩家可见日志和记录摘要，不改变 `ZoneDirective` 编译或规则执行。
...
tile.isCapturable
tile.controller != division.faction
tile.controller == nil，或 controller 与 division.faction 外交 hostile / atWar
destination 没有其他单位
```
...
元帅编译完成后，`TurnManager.applyRulerAdjustment` 会调用 `RulerAgent.adjust`。它可以根据当前压力、优势防区、外交敌对数量和近期防守记录，把进攻改为守成、调整进攻强度、提高防守预备队或重排目标 region；外交敌对数量来自 `DiplomacyState.hostileCountryIds`，相邻敌军强度来自外交 hostile 单位筛选。同时写入 `RulerDecisionRecord` 和外交日志。这个阶段只返回新的 `DirectiveEnvelope`，不执行底层命令，也不直接修改战术权威状态。
...
随后 `TurnManager.applyStrategistPlanning` 调用 `StrategistAgent.plan`。它承接君主姿态和太守上下文，选择主防区，重排攻击目标 region，补齐 focus/support/convergence 参数，并写入 `StrategistDecisionRecord`；其邻近敌对单位存在判断按 `DiplomacyState` hostile / atWar 过滤，region/controller 争夺语义保持不变。这个阶段仍只返回新的 `DirectiveEnvelope`，不执行底层命令，也不直接修改战术权威状态。
...
visibleEnemyStrengthByRegion
  - 只统计 DiplomacyState hostile / atWar 单位
...
frontPressure
roadHexCount
pressuredRoadHexCount（可见 hostile 军队邻近/占据的官道格数）
roadPressureSourceSummaries（可见 hostile 敌军造成官道受压的最近来源、道路坐标、距离和敌将）
friendlyDivisions
visibleEnemyDivisions（只统计 hostile 且可见）
visibleNonHostileDivisions（非己方、非 hostile 且可见）
friendlyGeneralSummaries（本郡己方军队对应武将姓名）
visibleEnemyEngagementSummaries（可见 hostile 军队距锚点、射程、兵力、敌将）
```
...
用户点下一回合 / AI faction active
  ...
  -> ZoneCommanderAgent 按 DiplomacyState hostile / atWar 口径计算兵力比/可见敌军/前沿存在
...
7. Directive 层
   ...
   - visibleEnemyStrength / friendlyFrontStrength 是否按 DiplomacyState 过滤且数值合理？
...
玩家宏观将军命令
  ...
  -> RuleEngine
  -> WarDirectiveRecord + PlayerPlannedOperation(tactic)
  -> BoardScene 计划线 / GeneralCommandPanelView 计划摘要 / 金色微操单位圈
```
...
- 记录与反馈：玩家宏观命令写入 `WarDirectiveRecord` 和 `PlayerPlannedOperation`，两者都记录最终 tactic。`BoardScene` 只读 `PlayerCommandState.plannedOperations`，画源 region 到目标 region 的箭头、源/目标点和武将/攻守/短战术标签，并基于源/目标代表 hex 标出官道、离道、可见敌军压迫和最近可见敌军短标签；防御命令画源点圆环、固守标签和源点官道/可见受压/近敌状态。`AppContainer.selectedGeneralPlannedOperationRows` 读取同一计划记录，为 `GeneralCommandPanelView` 的计划军令列表显示武将、指令类型、最终战术、源→目标、官道状态、源目可见受压和最近可见敌军对象/距离；该摘要只表示源/目标代表 hex 和当前最近可见敌军对象距离，不模拟完整行军路径。玩家微操锁定单位在 `UnitNode` 上显示金色底圈。
