...
- `AppContainer.interactionLog` 当前还负责玩家交互日志的三国化显示：基础命令写作命令已执行/被拒绝，武将宏观军令写作武将军令失败/已提交/已执行，规则拒绝、命令条数、手动指挥军队排除、查看/选择军队和选择地格/郡县均中文优先；这不改变底层命令、记录 id、Codable 或 rawValue。
...
`AppContainer.interactionLog` 的玩家可见文本已覆盖基础命令执行/拒绝、武将军令提交/拒绝、规则拒绝摘要、命令条数、手动指挥军队排除、查看/选择军队和选择地格/郡县的中文三国化显示；这些只是 `appendInteractionEvent(...)` 与私有展示 helper 的文本边界，`submitPlayerDirective(...) -> GeneralAgent.plan -> WarCommandExecutor -> RuleEngine` 的执行顺序、`WarDirectiveRecord`、`PlayerPlannedOperation`、Codable 字段和 rawValue 不变。
