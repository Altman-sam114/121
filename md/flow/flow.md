...
- 道路 ZOC、粮道上的敌对单位、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、动态前线敌对接触、Legacy AgentContext 敌军摘要、MockAICommander 威胁估算、ZoneCommanderAgent / MarshalBattlefieldSummarizer / RulerStrategicSnapshot / StrategistBattlefieldSnapshot 单位级 AI hostile 摘要、攻击目标、玩家高亮/点击、执行器单位目标和区域交战压力优先按 `DiplomacyState` 的 hostile / atWar 判断，缺外交建档时 fallback 到 `Faction.isHostile(to:)`；`OccupationRules.canOccupy` 也只允许占领无控制者或外交 hostile / atWar 控制格，AI / 武将执行器只把执行前真实可占领的移动作为动态方面突破依据，避免非敌对借道经过 allied / coBelligerent / neutral 控制格时翻转 controller 或推进动态归属；objective 和 region controller 来源、非同 faction 堆叠阻挡、前线压力/补给/包围拓扑和 encirclement 拓扑仍保留原语义。中立势力不会只因 `faction != activeFaction` 阻断道路、切断粮道、触发围城、阻止补员、生成敌对部署段、生成敌对前线、进入 AI 敌军摘要、被自动占领或被合法攻击。
