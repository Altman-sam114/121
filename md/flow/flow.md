...
- 道路敌控区、粮道上的敌对单位阻断、围城邻接、安全补员邻接、玩家地图点击攻击、攻击高亮、武将宏观目标选择、攻击合法性、执行器单位目标筛选、区域交战压力和郡县可见敌军展示优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺外交建档时 fallback 到 `Faction.isHostile(to:)`，避免中立、停战、同盟或共同作战势力被误判为可攻击、切粮、围城或阻止补员的敌军；领土 controller、非同 faction 堆叠阻挡和部署接触仍保留原控制权/阵营语义。
...
- `MovementRules.isEnemyZoneOfControl`、`SupplyRules` 粮道单位阻断和围城邻接、`EconomyRules` 安全补员邻接、`CommandValidator.validateAttack`、`RegionCommandValidator` 攻击目标校验、`CommandExecutor.executeAttack` 防绕过 guard、`WarCommandExecutor` 单位级敌军强度/存在/目标筛选、`RegionCombatRules.pressure`、`AppContainer` 的地图点击攻击/可见攻击高亮/武将宏观目标选择、郡县检查器的可见敌军/非敌对军队分组、官道受压计数和最近来源，`AppContainer` 的军队接战预判、武将麾下/目标/计划军令近敌与受压摘要、移动预览可见敌控过滤和点击日志敌对措辞，以及 `BoardScene` 计划军令地图短标签、`CommandPanelView` 状态文案、`GeneralCommandPanelView` 目标预览显示 gate，优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺建档时 fallback 到 `Faction.isHostile(to:)`；`SupplyRules` 的 capturable tile controller、`MovementRules` 的非同 faction 堆叠阻挡、`WarDeploymentManager` 单位级敌军存在和相邻敌对防区接触仍使用 `Faction.isHostile(to:)` 或控制权语义，非己方控制 hex/region 的部署压力分类仍表示未控制或争夺，不等同于敌军单位；完整借道/同盟通行仍待后续外交制度。
...
- 道路 ZOC、粮道上的敌对单位、围城邻接、安全补员邻接、攻击目标、玩家高亮/点击、执行器单位目标和区域交战压力优先按 `DiplomacyState` 的 hostile / atWar 判断，缺外交建档时 fallback 到 `Faction.isHostile(to:)`；capturable tile controller、非同 faction 堆叠阻挡和部署/占领控制权仍保留原语义。中立势力不会只因 `faction != activeFaction` 阻断道路、切断粮道、触发围城、阻止补员或被合法攻击。
