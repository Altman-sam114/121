...
- `AppContainer.selectedGeneralInfluenceNotes` 复用 `GeneralInfluence.movementSummary`、`GeneralInfluence.combatSummary` 和当前军队坐标，为 `GeneralCommandPanelView` 提供当前防区道路机动、接敌攻防、当前接敌配对与最近敌军对象只读摘要；它不执行命令，也不修改道路、军队、控制权或动态战区。
