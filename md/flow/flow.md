...
- `GeneralAssignment` 现在保存武将姓名、风格和技能快照；`GeneralSkillDisplay` 将 raw skill id 显示为粮道调度、骑兵突击、守备专精等中文标签，并为 UI 提供官道机动、骑兵突击、攻城修正、地形/渡河防御等只读短效果提示，规则和 JSON 仍保留原 id；`GeneralAgent` 会读取这些快照，把军师后的 `ZoneDirective.tactic` 收束为攻守类别合法、且符合机动/器械/预备队条件的战术；`GeneralInfluence` 会读取防区武将分配，给道路机动、攻击和防御提供小幅规则修正。
