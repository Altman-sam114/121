...
- `RuleEngine`、`AppContainer`、`CommandResultSummary` 和 `AgentPanelView` 的玩家/AI 可见命令结果展示当前已收敛：成功 `CommandResult.message` 使用 state-aware 中文摘要，玩家交互日志不再重复拼接 raw id 版 `Command.displayName`，AI 面板命令结果标题只显示命令类别；`Command.displayName`、`CommandResultSummary.commandDisplayName` 原始字段和旧 AI 兼容判断仍保持不变。
...
这条路径中的交互日志只做展示：成功/拒绝状态写作“命令已执行/命令被拒绝”，命令本身仍来自 `Command.displayName` 和 `CommandResult.message`。
