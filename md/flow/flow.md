...
- 记录与反馈：玩家宏观命令写入 `WarDirectiveRecord` 和 `PlayerPlannedOperation`，两者都记录最终 tactic。`BoardScene` 只读 `PlayerCommandState.plannedOperations`，画源 region 到目标 region 的箭头、源/目标点和武将/攻守/短战术标签，并基于源/目标代表 hex 标出官道、离道和敌军压迫短标签；防御命令画源点圆环、固守标签和源点官道/受压状态。`AppContainer.selectedGeneralPlannedOperationRows` 读取同一计划记录，为 `GeneralCommandPanelView` 的计划军令列表显示武将、指令类型、最终战术、源→目标、官道状态、源目受压和最近敌军对象/距离；该摘要只表示源/目标代表 hex 和当前最近敌军对象距离，不模拟完整行军路径。玩家微操锁定单位在 `UnitNode` 上显示金色底圈。
