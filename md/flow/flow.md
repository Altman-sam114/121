...
- 道路敌控区、粮道上的敌对单位阻断、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、玩家地图点击攻击、攻击高亮、武将宏观目标选择、攻击合法性、执行器单位目标筛选、区域交战压力和郡县可见敌军展示优先使用 `DiplomacyState` 的 `DiplomaticRelation.status`，缺外交建档时 fallback 到 `Faction.isHostile(to:)`，避免中立、停战、同盟或共同作战势力被误判为可攻击、切粮、围城、阻止补员或生成敌对部署接触的敌军；领土 controller、非同 faction 堆叠阻挡、非己方控制 hex / region 的部署压力分类和 encirclement 拓扑仍保留原控制权/阵营语义。
...
- 道路 ZOC、粮道上的敌对单位、围城邻接、安全补员邻接、部署层敌军存在、相邻敌对防区接触、攻击目标、玩家高亮/点击、执行器单位目标和区域交战压力优先按 `DiplomacyState` 的 hostile / atWar 判断，缺外交建档时 fallback 到 `Faction.isHostile(to:)`；capturable tile controller、非同 faction 堆叠阻挡、部署/占领控制权和 encirclement 拓扑仍保留原语义。中立势力不会只因 `faction != activeFaction` 阻断道路、切断粮道、触发围城、阻止补员、生成敌对部署段或被合法攻击。
