# AGENTS.md
本文是“三国棋策 Agent”（仍沿用 WWIIHexV0 工程名）的入口记忆、总览、基本规则和多 Agent 工作流。任何 Agent 接手任务时，先读本文，再读任务所需文档和源码；不要凭旧 prompt、旧记忆或猜测修改项目。
## 1. 必读文件
...
6. 当前目标对应的 prompt / 阶段文档；提示词索引见 `md/prompt/README.md`。
7. 相关源码、配置和必要时的测试文件：优先用 `rg` / `rg --files` 定位；测试文件默认只作语义参考，不默认执行。
若文档、源码、轻量检查结果冲突，以当前源码和真实检查结果为准，并在本轮结束时同步修正文档。
## 2. 项目基本规则
- 本项目是 Swift + SwiftUI + SpriteKit 的 iOS / macOS 回合制 hex 战棋，当前处于 v2.4 君主/外交/太守/军师/武将指令编排、外交与太守生产命令、道路和交战兼容层；源码目录和部分 Codable/rawValue 仍保留 WWIIHexV0 / germany / allies 等兼容名，默认加载已优先使用官渡预览剧本和 `sanguo_unit_templates.json`，旧阿登数据和旧 `unit_templates.json` 保留作 fallback 和回归参考。
...
- 没有这些前缀时，按普通 Codex 任务处理；若任务天然需要 A/B/C 边界，先说明本轮按普通任务执行，或提醒人工指定角色。
### 3.1 人工
...
6. 本地轻量检查通过后，提交并直接 push 到 `origin/main`，触发 GitHub Actions 云端重验证。
7. 输出实现结果：改动摘要、关键文件、轻量检查命令和结果、push 的 commit、未跑本机重测试及原因、等待 Agent C 复判的云端结果。
### 3.4 Agent C：云端结果包验收与核心逻辑文档
...
2. 使用 `gh auth login` 后下载 `origin/main` 最新 run 的未加密 CI 结果包，缓存到 `/private/tmp/three-kingdoms-agent-c-review-<run_id>/`，不要自动删除。
3. 核对 artifact 中 `ci-artifact-manifest.json` 的 `branch`、`commitSha`、`runId`、`runAttempt` 与 `origin/main` 最新 commit / run 完全一致，并阅读 `ci-failure-summary.md`、`junit.xml`、主构建日志和 `.xcresult`（如存在）。
...
8. 输出验收结论：通过/不通过、问题清单、artifact 核对结果、已更新文档、轻量检查结果、建议下一步。
### 3.5 回到人工
人工阅读 Agent C 的验收、核心逻辑文档和轻量检查结果，决定是否接受、授权补测、修正、合并分支或进入下一轮开发。下一轮通过 `update_log.md` 和新的目标继续交给 Agent A，形成循环迭代。
### 3.6 main 直推与多 Agent 规则
- 当前默认只使用 `main` 作为上传、提交、推送和云端验证分支；暂不使用 `smalldata_test`、`develop`、`codeb/...` 或 PR 流程。
- 若历史分支仍存在，只记录现状；不得把它们写成本轮默认协作制度。
- Agent B 每轮开始前推荐执行：`git fetch origin`、`git switch main`、`git pull --ff-only origin main`、`git status --short`。
...
- 后续如人工明确要求多分支候选，每个分支必须记录：目标、范围、关键文件、与其他分支的预期重叠、轻量检查结果和未确认风险。
...
- 现在默认本机不做 Xcode / XCTest / 模拟器测试，也不做耗费性能的构建验证；重验证默认交给 GitHub Actions。
- 禁止在本机主动运行：`xcodebuild test`、`xcodebuild build-for-testing`、Probe、Smoke、Stage Regression、Dynamic Theater Regression、Full、UI test、性能测试、模拟器启动和全量 app 构建。
- 默认只做轻量语法/格式检查：Markdown 文本检查、`plutil -lint`、`xmllint --noout`、`jq empty`、针对改动文件的轻量静态检查等，具体以 `md/test/test.md` 为准。
- 若任务风险必须靠重测试才能排除，Agent 只能在交付中说明“按当前规范未跑本机重测试，等待或查看云端 CI”，不得自行扩大本机验证。
...
- `.github/workflows/ci-results.yml` 记录云端重验证和 Agent C 结果包产出；结果包必须未加密，不能复用带密码发布包。
