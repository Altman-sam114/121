...
## 2. 项目基本规则
- 本项目是 Swift + SwiftUI + SpriteKit 的 iOS / macOS 回合制 hex 战棋，当前处于 v2.4 君主姿态塑形兼容层；源码目录和部分 Codable/rawValue 仍保留 WWIIHexV0 / germany / allies 等兼容名，默认加载已优先使用官渡预览剧本和 `sanguo_unit_templates.json`，旧阿登数据和旧 `unit_templates.json` 保留作 fallback 和回归参考。
