...
struct MockAIClient: DecisionProvider {
    func decide(context: AgentContext) async throws -> AgentDecisionEnvelope {
            ...
            if division.supplyState == .lowSupply || division.supplyState == .encircled {
                orders.append(
                    AgentOrder(
                        ...
                        stance: "recover",
                        reason: "军队处于\(division.supplyState.displayName)，先整补粮草再继续进攻。"
                    )
                )
                continue
            }
               ...
               let destination = bestMoveDestination(
                ...
                orders.append(
                    AgentOrder(
                        ...
                        reason: "向\(objective.name)推进，优先借官道和通畅路线机动。"
                    )
                )
                continue
            }
        ...
        return AgentDecisionEnvelope(
            ...
            turn: context.turn,
            intent: "沿官道集中机动力量推进，并等待器械支援后突破要地。",
            orders: orders
        )
    }
    ...
    private func frontDeploymentDecision(context: AgentContext) -> AgentDecisionEnvelope? {
            ...
            if division.supplyState == .lowSupply || division.supplyState == .encircled {
                orders.append(
                    AgentOrder(
                        ...
                        reason: "部署兼容层：军队处于\(division.supplyState.displayName)，先整补粮草再参与前线行动。"
                    )
                )
                   ...
                   regionControllers[targetRegion] == context.faction {
                    ...
                    orders.append(
                        AgentOrder(
                            ...
                            toRegionId: division.regionId,
                            stance: "depthReserve",
                            reason: "部署兼容层：纵深预备队暂无相邻安全前线目标，继续待命。"
                        )
                    )
                }
                usedDivisionIds.insert(unitId)
        ...
        return AgentDecisionEnvelope(
            ...
            agentId: context.agentId,
            turn: context.turn,
            intent: "按兼容防区部署执行：前线军队固守或接战，纵深预备队支援，守备军守城。",
            orders: orders
        )
    }
